...
std::pair<uint32_t, std::string> TraCICommandInterface::getVersion() {
  bool success = false;
  TraCIBuffer buf =
      connection.queryOptional(CMD_GETVERSION, TraCIBuffer(), success);
...
void TraCICommandInterface::Vehicle::setSpeedMode(int32_t bitset) {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId
                                              << variableType << bitset);
  ASSERT(buf.eof());
}
...
void TraCICommandInterface::Vehicle::slowDown(double speed, int32_t time) {
  ...
  TraCIBuffer buf = connection->query(
      ...
      TraCIBuffer() << variableId << nodeId << variableType << count
                    << speedType << speed << durationType << time);
  ASSERT(buf.eof());
}
...
std::list<std::string> TraCICommandInterface::getVehicleTypeIds() {
  return genericGetStringList(CMD_GET_VEHICLETYPE_VARIABLE, "", ID_LIST,
...
double TraCICommandInterface::Road::getCurrentTravelTime() {
  return traci->genericGetDouble(CMD_GET_EDGE_VARIABLE, roadId,
...
std::string TraCICommandInterface::Vehicle::getRoadId() {
  return traci->genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ROAD_ID,
                                 RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string TraCICommandInterface::Vehicle::getCurrentRoadOnRoute() {
  return traci->genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, LANE_EDGE_ID,
                                 RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string TraCICommandInterface::Vehicle::getLaneId() {
  return traci->genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_LANE_ID,
                                 RESPONSE_GET_VEHICLE_VARIABLE);
}

int32_t TraCICommandInterface::Vehicle::getLaneIndex() {
  return traci->genericGetInt(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_LANE_INDEX,
                              RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string TraCICommandInterface::Vehicle::getTypeId() {
  return traci->genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_TYPE,
                                 RESPONSE_GET_VEHICLE_VARIABLE);
}

double TraCICommandInterface::Vehicle::getLanePosition() {
  return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId,
                                 VAR_LANEPOSITION,
                                 RESPONSE_GET_VEHICLE_VARIABLE);
}

std::list<std::string> TraCICommandInterface::Vehicle::getPlannedRoadIds() {
  return traci->genericGetStringList(CMD_GET_VEHICLE_VARIABLE, nodeId,
                                     VAR_EDGES, RESPONSE_GET_VEHICLE_VARIABLE);
}

std::string TraCICommandInterface::Vehicle::getRouteId() {
  return traci->genericGetString(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ROUTE_ID,
                                 RESPONSE_GET_VEHICLE_VARIABLE);
}
...
std::list<std::string> TraCICommandInterface::getRoadIds() {
  return genericGetStringList(CMD_GET_EDGE_VARIABLE, "", ID_LIST,
                              RESPONSE_GET_EDGE_VARIABLE);
}
...
void TraCICommandInterface::Vehicle::changeRoute(std::string roadId,
  ...
  if (travelTime >= 0) {
    ...
    std::string edgeId = roadId;
    TraCIBuffer buf =
        connection->query(CMD_SET_VEHICLE_VARIABLE,
                          TraCIBuffer() << variableId << nodeId << variableType
...
double TraCICommandInterface::getDistance(const Coord &p1, const Coord &p2,
  ...
  TraCIBuffer buf =
      connection.query(CMD_GET_SIM_VARIABLE,
                       TraCIBuffer() << variable << simId << variableType
...
void TraCICommandInterface::Vehicle::stopAt(std::string roadId, double pos,
                                            uint8_t laneid, double radius,
                                            double waittime) {
  ...
  TraCIBuffer buf = connection->query(
      CMD_SET_VEHICLE_VARIABLE,
      TraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT
                    << edgeId << stopPosT << stopPos << stopLaneT << stopLane
                    << durationT << duration);
  ASSERT(buf.eof());
}
...
std::string TraCICommandInterface::Polygon::getTypeId() {
  return traci->genericGetString(CMD_GET_POLYGON_VARIABLE, polyId, VAR_TYPE,
                                 RESPONSE_GET_POLYGON_VARIABLE);
}

std::list<Coord> TraCICommandInterface::Polygon::getShape() {
  return traci->genericGetCoordList(CMD_GET_POLYGON_VARIABLE, polyId, VAR_SHAPE,
                                    RESPONSE_GET_POLYGON_VARIABLE);
}
...
void TraCICommandInterface::addPolygon(std::string polyId, std::string polyType,
                                       const TraCIColor &color, bool filled,
                                       int32_t layer,
...
void TraCICommandInterface::addPoi(std::string poiId, std::string poiType,
                                   const TraCIColor &color, int32_t layer,
                                   const Coord &pos_) {
  TraCIBuffer p;
...
std::list<Coord> TraCICommandInterface::Lane::getShape() {
  return traci->genericGetCoordList(CMD_GET_LANE_VARIABLE, laneId, VAR_SHAPE,
                                    RESPONSE_GET_LANE_VARIABLE);
}

std::string TraCICommandInterface::Lane::getRoadId() {
  return traci->genericGetString(CMD_GET_LANE_VARIABLE, laneId, LANE_EDGE_ID,
                                 RESPONSE_GET_LANE_VARIABLE);
}

double TraCICommandInterface::Lane::getLength() {
  return traci->genericGetDouble(CMD_GET_LANE_VARIABLE, laneId, VAR_LENGTH,
                                 RESPONSE_GET_LANE_VARIABLE);
}

double TraCICommandInterface::Lane::getMaxSpeed() {
  return traci->genericGetDouble(CMD_GET_LANE_VARIABLE, laneId, VAR_MAXSPEED,
                                 RESPONSE_GET_LANE_VARIABLE);
}

double TraCICommandInterface::Lane::getMeanSpeed() {
  return traci->genericGetDouble(CMD_GET_LANE_VARIABLE, laneId,
                                 LAST_STEP_MEAN_SPEED,
                                 RESPONSE_GET_LANE_VARIABLE);
}
...
Coord TraCICommandInterface::Junction::getPosition() {
  return traci->genericGetCoord(CMD_GET_JUNCTION_VARIABLE, junctionId,
                                VAR_POSITION, RESPONSE_GET_JUNCTION_VARIABLE);
}
...
bool TraCICommandInterface::addVehicle(std::string vehicleId,
                                       ...
                                       double emitPosition, double emitSpeed,
                                       int8_t emitLane) {
  bool success = false;
  ...
  TraCIBuffer buf = connection.queryOptional(
      CMD_SET_VEHICLE_VARIABLE,
      TraCIBuffer() << variableId << vehicleId << variableType << count
                    << (uint8_t)TYPE_STRING << vehicleTypeId
                    << (uint8_t)TYPE_STRING << routeId << (uint8_t)TYPE_INTEGER
                    << emitTime << (uint8_t)TYPE_DOUBLE << emitPosition
...
void TraCICommandInterface::GuiView::setScheme(std::string name) {
  TraCIBuffer buf = connection->query(
      ...
      TraCIBuffer() << static_cast<uint8_t>(VAR_VIEW_SCHEMA) << viewId
                    << static_cast<uint8_t>(TYPE_STRING) << name);
  ASSERT(buf.eof());
}

void TraCICommandInterface::GuiView::setZoom(double zoom) {
  TraCIBuffer buf = connection->query(
      CMD_SET_GUI_VARIABLE, TraCIBuffer()
                                << static_cast<uint8_t>(VAR_VIEW_ZOOM) << viewId
                                << static_cast<uint8_t>(TYPE_DOUBLE) << zoom);
  ASSERT(buf.eof());
}
...
void TraCICommandInterface::GuiView::setBoundary(Coord p1_, Coord p2_) {
  ...
  TraCIBuffer buf = connection->query(
      CMD_SET_GUI_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_VIEW_BOUNDARY) << viewId
                    << static_cast<uint8_t>(TYPE_BOUNDINGBOX) << p1.x << p1.y
                    << p2.x << p2.y);
  ASSERT(buf.eof());
}
...
void TraCICommandInterface::GuiView::takeScreenshot(std::string filename) {
  if (filename == "") {
    ...
    char ss[512];
    snprintf(ss, sizeof(ss), "%s/screenshot-%s-@%08.2f.png",
             absolutePath.c_str(), myRunID, simTime().dbl());
    filename = ss;
  }

  TraCIBuffer buf = connection->query(
      CMD_SET_GUI_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_SCREENSHOT) << viewId
                    << static_cast<uint8_t>(TYPE_STRING) << filename);
  ASSERT(buf.eof());
}
...
std::string TraCICommandInterface::genericGetString(uint8_t commandId,
  ...
  TraCIBuffer buf =
      connection.query(commandId, TraCIBuffer() << variableId << objectId);
...
std::list<std::string> TraCICommandInterface::genericGetStringList(
...
std::list<Coord> TraCICommandInterface::genericGetCoordList(
    uint8_t commandId, std::string objectId, uint8_t variableId,
    uint8_t responseId) {
...
void TraCICommandInterface::Vehicle::getVehicleData(
    double &speed, double &acceleration, double &controllerAcceleration,
    double &positionX, double &positionY, double &time) {

  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_SPEED_AND_ACCELERATION)
                    << nodeId);
...
void TraCICommandInterface::Vehicle::setGenericInformation(int type,
                                                           const void *data,
                                                           int length) {
...
void TraCICommandInterface::Vehicle::getGenericInformation(int type,
                                                           const void *params,
                                                           int paramsLength,
                                                           void *result) {
...
double TraCICommandInterface::Vehicle::getACCAcceleration() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_ACC_ACCELERATION)
                    << nodeId);
...
void TraCICommandInterface::Vehicle::setPlatoonLeaderData(double speed,
                                                          double acceleration,
                                                          double positionX,
                                                          double positionY,
                                                          double time) {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << speed
                                              << acceleration << positionX
                                              << positionY << time);
  ASSERT(buf.eof());
}

void TraCICommandInterface::Vehicle::setPrecedingVehicleData(
    double speed, double acceleration, double positionX, double positionY,
    double time) {
...
unsigned int TraCICommandInterface::Vehicle::getLanesCount() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_LANES_COUNT) << nodeId);
...
void TraCICommandInterface::Vehicle::setCruiseControlDesiredSpeed(
    double desiredSpeed) {
...
int TraCICommandInterface::Vehicle::getActiveController() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_ACTIVE_CONTROLLER)
                    << nodeId);
...
void TraCICommandInterface::Vehicle::setCACCConstantSpacing(double spacing) {
  ...
  TraCIBuffer buf =
      traci->connection.query(CMD_SET_VEHICLE_VARIABLE,
                              TraCIBuffer() << variableId << nodeId << spacing);
  ASSERT(buf.eof());
}
double TraCICommandInterface::Vehicle::getCACCConstantSpacing() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_CACC_SPACING) << nodeId);
...
void TraCICommandInterface::Vehicle::setACCHeadwayTime(double headway) {
  ...
  TraCIBuffer buf =
      traci->connection.query(CMD_SET_VEHICLE_VARIABLE,
                              TraCIBuffer() << variableId << nodeId << headway);
  ASSERT(buf.eof());
}

void TraCICommandInterface::Vehicle::setFixedAcceleration(int activate,
                                                          double acceleration) {
...
bool TraCICommandInterface::Vehicle::isCrashed() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_CRASHED) << nodeId);
...
bool TraCICommandInterface::Vehicle::isCruiseControllerInstalled() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_CC_INSTALLED) << nodeId);
...
void TraCICommandInterface::Vehicle::setLaneChangeAction(int action) {
  ...
  TraCIBuffer buf =
      traci->connection.query(CMD_SET_VEHICLE_VARIABLE,
                              TraCIBuffer() << variableId << nodeId << action);
  ASSERT(buf.eof());
}
...
int TraCICommandInterface::Vehicle::getLaneChangeAction() {
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_LANE_CHANGE_ACTION)
                    << nodeId);
...
void TraCICommandInterface::Vehicle::getRadarMeasurements(
    double &distance, double &relativeSpeed) {

  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_RADAR_DATA) << nodeId);
...
void TraCICommandInterface::Vehicle::setControllerFakeData(
    double frontDistance, double frontSpeed, double frontAcceleration,
    double leaderSpeed, double leaderAcceleration) {

  uint8_t variableId = VAR_SET_CONTROLLER_FAKE_DATA;
  TraCIBuffer buf = traci->connection.query(
      CMD_SET_VEHICLE_VARIABLE,
      TraCIBuffer() << variableId << nodeId << frontDistance << frontSpeed
                    << frontAcceleration << leaderSpeed << leaderAcceleration);
  ASSERT(buf.eof());
}
...
double TraCICommandInterface::Vehicle::getDistanceFromRouteBegin() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_DISTANCE_FROM_BEGIN)
                    << nodeId);
...
double TraCICommandInterface::Vehicle::getDistanceToRouteEnd() {
  ...
  TraCIBuffer buf = traci->connection.query(
      CMD_GET_VEHICLE_VARIABLE,
      TraCIBuffer() << static_cast<uint8_t>(VAR_GET_DISTANCE_TO_END) << nodeId);
