...
public:
  ...
  double getDistance(const Coord &position1, const Coord &position2,
                     bool returnDrivingDistance);

  // Vehicle methods
  bool addVehicle(std::string vehicleId, std::string vehicleTypeId,
  ...
  public:
    ...
    void stopAt(std::string roadId, double pos, uint8_t laneid, double radius,
                double waittime);
    int32_t getLaneIndex();
    std::string getTypeId();
    bool changeVehicleRoute(const std::list<std::string> &roads);
    /**
     * Gets the total number of lanes on the edge the vehicle is currently
     * traveling
     */
     ...
     */
    void setPlatoonLeaderData(double leaderSpeed, double leaderAcceleration,
                              double positionX, double positionY, double time);
    /**
     * Sets the data about the preceding vehicle in the platoon. This data is
     * usually
     * received by means of wireless communications
     */
    void setPrecedingVehicleData(double speed, double acceleration,
                                 double positionX, double positionY,
                                 double time);
    /**
     * Gets the data about a vehicle. This can be used by a platoon leader in
     * order to query for the acceleration
     * before sending the data to the followers
     */
    void getVehicleData(double &speed, double &acceleration,
                        double &controllerAcceleration, double &positionX,
                        double &positionY, double &time);

    void setGenericInformation(int type, const void *data, int length);
    void getGenericInformation(int type, const void *params, int paramsLength,
                               void *result);
    ...
    /**
     * Set the currently active controller, which can be either the driver, the
     * ACC or
     * the CACC. CC is not mentioned because CC and ACC work together
     *
     * @param vehicleId the id of vehicle for which the active controller must
     * be set
     * @param activeController the controller to be activated: 0 for driver, 1
     * for
     * ACC and 2 for CACC
     */
     ...
     *
     * @param vehicleId the id of vehicle for which the constant spacing must be
     * set
     ...
     *
     * @param vehicleId the id of the vehicle
     * @param activate activate (1) or deactivate (0) the usage of a fixed
     * acceleration
    ...
    /**
     * Tells whether the car has an ACC/CACC controller installed or not.
     * Basically
    ...
    /**
     * Tells to the CC mobility model the desired lane change action to be
     * performed
     *
     * @param vehicleId the vehicle id to communicate the action to
     * @param action the action to be performed. this can be either:
     * 0 = driver choice: the application protocol wants to let the driver chose
     * the lane
     * 1 = management lane: the application protocol wants the driver to move
     * the car
     * to the management lane, i.e., the leftmost minus one
     * 2 = platooning lane: the application protocol wants the driver to move
     * the car
    ...
    void setControllerFakeData(double frontDistance, double frontSpeed,
                               double frontAcceleration, double leaderSpeed,
  ...
  void addPolygon(std::string polyId, std::string polyType,
  ...
  // Poi methods
  void addPoi(std::string poiId, std::string poiType, const TraCIColor &color,
              int32_t layer, const Coord &pos);
  class Poi {
  public:
    Poi(TraCICommandInterface *traci, std::string poiId)
        : traci(traci), poiId(poiId) {
      connection = &traci->connection;
    }
  ...
  public:
    Junction(TraCICommandInterface *traci, std::string junctionId)
        : traci(traci), junctionId(junctionId) {
      connection = &traci->connection;
    }
...
private:
  ...
  double genericGetDouble(uint8_t commandId, std::string objectId,
                          uint8_t variableId, uint8_t responseId);
  int32_t genericGetInt(uint8_t commandId, std::string objectId,
                        uint8_t variableId, uint8_t responseId);
  std::list<std::string> genericGetStringList(uint8_t commandId,
                                              std::string objectId,
                                              uint8_t variableId,
                                              uint8_t responseId);
  std::list<Coord> genericGetCoordList(uint8_t commandId, std::string objectId,
                                       uint8_t variableId, uint8_t responseId);
};
}
